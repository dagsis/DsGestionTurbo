VERSION 5.00
Begin VB.Form TblUniMedida 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla de Unidad de Medidas"
   ClientHeight    =   3525
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6510
   ...
   Begin VB.Frame Frame1 
      Caption         =   "ABM de Unidad de Medida"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "TblUniMedida"
...
Private Sub LinkearTexto()
  Label3.Caption = Format(Rs!unidad, "00")
...
Private Sub List1_Click()
 ...
 If sCod <> 0 Then
    Set Rs = cRsl.TraerRS("UnidadTraerUno", sCod, True)
    LinkearTexto
 Else
    MsgBox "Esta Unidad no se puede Utilizar", vbCritical, "Atención"
 End If
 Set cRsl = Nothing
Exit Sub
errHandler:
...
Private Sub Borrar()
  ...
  If cRsl.TraerCantidad("Productos", "UMVenta", "UmVenta=" & Val(Label3.Caption)) = 0 And cRsl.TraerCantidad("Registros", "Medida", "Medida=" & Val(Label3.Caption)) = 0 Then
    If MsgBox("Borra La Unidad de Medida Seleccionada ?", 20, "Atención") = 6 Then
       sCod = Label3.Caption
       cRse.Actualizar "UnidadBorrar", Nothing, Val(Label3.Caption), "Medida"
       CargarLista
       Limpiar
       Botones True, False, False, False, True
        If Rs.EOF = False Or Rs.BOF = False Then
    ...
    End If
  Else
     MsgBox "Unidad de Medida Asociada a un producto o es un Registro por defecto", vbCritical, "Atención"
  End If
  Exit Sub
errHandler:
  ManejaErrores
...
Private Sub CargarLista()
  ...
  List1.Clear
  Set Rs = cRsl.TraerRS("UnidadTraerTodos", , True)

  Do While Not Rs.EOF
     List1.AddItem Rs!Descripcion
     List1.ItemData(List1.NewIndex) = Rs!unidad
     Rs.MoveNext
  Loop
  Set cRsl = Nothing
  Exit Sub
...
Private Sub Grabar()
  ...
  If CmdBotones(2).Caption = "Grabar" Then
     cRse.Actualizar "UnidadModif", Rs1, Val(Label3.Caption)
  Else
     cRse.Actualizar "UnidadAgregar", Rs1
  End If
  Rs1.Close
  Set cRse = Nothing
  Exit Sub
