...
Begin VB.Form ComprasAplicacion 
   ...
   End
End
Attribute VB_Name = "ComprasAplicacion"
...
Private Sub CmdBotones_Click(Index As Integer)
  ...
  Select Case Index
         ...
         Case 1
              ...
              bApli = False
              If cRx.TieneAplicacion(ComprasComprobantes.CmbComprobante.ItemData(ComprasComprobantes.CmbComprobante.ListIndex), Right(LblNumero.Caption, 8), ComprasComprobantes.TxtCliente.text) <> 0 Then
                 bApli = True
                 Me.Caption = "Nota de Credito Aplicada"
...
Private Sub LlenarGrilla()
 ...
 Set rCuen = cCom.DameCuentaCorrienteP(ComprasComprobantes.TxtCliente.text)
...
Private Sub Form_Load()
  ...
  Botones True, False
  bApli = False
  cApli = cRx.TieneAplicacion(ComprasComprobantes.CmbComprobante.ItemData(ComprasComprobantes.CmbComprobante.ListIndex), Right(LblNumero.Caption, 8), ComprasComprobantes.TxtCliente.text)
  If cApli <> 0 Then
     bApli = True
     Me.Caption = "Nota de Credito Aplicada"
  End If
  ...
  lblTotal.Caption = Format(CCur(ComprasComprobantes.lblTotal.Caption) * CCur(ComprasComprobantes.TxtCotizacion.text) - cApli, "#0.00")
End Sub
...
Private Sub CargarCombos()
  ...
  LblComprobante.Caption = ComprasComprobantes.CmbComprobante.text
  LblNumero.Caption = ComprasComprobantes.TxtSucursal.text & "-" & ComprasComprobantes.TxtNumero.text
Exit Sub
...
Private Sub GrabarTodo()
 ...
 ' Grabar Valores
  ...
  Dim nCotizacion As Currency
    
  cDife = CCur(lblTotal.Caption)
  RsC.MoveFirst
  
  Dim nCat As Byte, nComp As Integer
  
  nComp = ComprasComprobantes.CmbComprobante.ItemData(ComprasComprobantes.CmbComprobante.ListIndex)
  
  nCat = cRsl.TraerValorDeUnCampo("Proveedores", "CatGanancia", "Proveedor='" & ComprasComprobantes.TxtCliente.text & "'")
  
  Do While Not RsC.EOF
     If RsC!Cancelado > 0 Then
        rApli.AddNew
        rApli!Venta = RsC!Venta
        rApli!MoviCta = ComprasComprobantes.nMovi
        rApli!Comprobante = nComp
        rApli!Numero = Right(LblNumero.Caption, 8)
        rApli!Sucursal = cNum.TraerSucursalNumero(ComprasComprobantes.CmbComprobante.ItemData(ComprasComprobantes.CmbComprobante.ListIndex))
        rApli!Cuota = RsC!Cuota
        rApli!importe = RsC!Cancelado
        rApli!Anulado = 0
        rApli.Update
        
        If nCat <> 5 And cRsl.TraerValorDeUnCampo("Comprobantes", "Iva", "Id=" & nComp) = 1 Then
           RsGan.AddNew
           RsGan!Movimiento = RsC!Venta
           RsGan!Proveedor = ComprasComprobantes.TxtCliente.text
           RsGan!fecha = DtpFecha.Value
           RsGan!importe = RsC!Cancelado
           RsGan!Retenido = 0
           RsGan!Anulado = False
           RsGan.Update
        End If
