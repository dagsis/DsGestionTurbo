VERSION 5.00
Begin VB.Form TblLocalidades 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tablas de Localidades"
   ClientHeight    =   3525
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6510
   ...
   End
End
Attribute VB_Name = "TblLocalidades"
...
Private Sub LinkearTexto()
  Label3.Caption = Format(rs!Localidad, "00")
...
Private Sub List1_Click()
 ...
 Set rs = cRsl.TraerRS("LocalidadTraerUno", sCod, True)
 LinkearTexto
 Set cRsl = Nothing
Exit Sub
errHandler:
...
Private Sub Borrar()
  ...
  If MsgBox("Borra el Banco Seleccionado ?", 20, "Atención") = 6 Then
     sCod = Label3.Caption
     cRse.Actualizar "LocalidadBorrar", rs, Val(Label3.Caption), "Localidad"
     CargarLista
     Limpiar
     Botones True, False, False, False, True
      If rs.EOF = False Or rs.BOF = False Then
...
Private Sub CargarLista()
  ...
  List1.Clear
  Set rs = cRsl.TraerRS("LocalidadTraerTodos", , True)

  Do While Not rs.EOF
     List1.AddItem rs!Descripcion
     List1.ItemData(List1.NewIndex) = rs!Localidad
     rs.MoveNext
  Loop
  Set cRsl = Nothing
  Exit Sub
...
Private Sub Grabar()
  ...
  If CmdBotones(2).Caption = "Grabar" Then
     cRse.Actualizar "LocalidadModif", Rs1, Val(Label3.Caption)
  Else
     cRse.Actualizar "LocalidadAgregar", Rs1
  End If
  Rs1.Close
  bLocalidad = True
  Set cRse = Nothing
