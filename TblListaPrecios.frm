VERSION 5.00
Begin VB.Form TblListaPrecios 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "ABM de Lista de Precio"
   ClientHeight    =   3525
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6510
   ...
   Begin VB.Frame Frame1 
      Caption         =   "ABM de Lista de Precios"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "TblListaPrecios"
...
Private Sub LinkearTexto()
  Label3.Caption = Format(Rs!ListaPrecio, "00")
  TxtDescripcion.text = Rs!Descripcion
  TxtPorcentaje.text = Format(Rs!Porcentaje, nCantDecimales)
  ChkPrecio.Value = Rs!PrecioIva
End Sub
...
Private Sub List1_Click()
 ...
 Set Rs = cRsl.TraerRS("ListaPrecioTraerUno", sCod, True)
 LinkearTexto
 Set cRsl = Nothing
Exit Sub
errHandler:
...
Private Sub Nuevo()
  ...
  Else
     ...
     Grabar
     Set cRsl = New ClsLectura
     Set Rs = cRsl.TraerRS("ListaPrecioTraerTodosN", , True)
     Rs.MoveLast
     Set Rs2 = New ADODB.Recordset
     Set Rs2 = cRsl.TraerRS("ProductoTraerTodos", , True)
     Set cRse = New ClsEscritura
     cRse.ActualizarPrecios Rs!ListaPrecio, Rs2
...
Private Sub Borrar()
  ...
  If cRx.TraerCantidad("Clientes", "Cliente", "Precio=" & Label3.Caption) = 0 _
          And cRx.TraerCantidad("CabComprobantes", "Lista", "Lista=" & Label3.Caption) = 0 Then
      If MsgBox("Borra La Lista de Precio Seleccionada ?", 20, "Atención") = 6 Then
        sCod = Label3.Caption
        cRse.Actualizar "ListaPrecioBorrar", Nothing, Val(Label3.Caption), "ListaPrecio"
        CargarLista
        Limpiar
        Botones True, False, False, False, True
         If Rs.EOF = False Or Rs.BOF = False Then
     ...
     End If
  Else
     MsgBox "La lista de Precio Tiene Movimientos Asociados", vbCritical, "Atención"
  End If
  Exit Sub
errHandler:
  ManejaErrores
...
Private Sub CargarLista()
  ...
  List1.Clear
  Set Rs = cRsl.TraerRS("ListaPrecioTraerTodos", , True)

  Do While Not Rs.EOF
     List1.AddItem Rs!Descripcion
     List1.ItemData(List1.NewIndex) = Rs!ListaPrecio
     Rs.MoveNext
  Loop
  Set cRsl = Nothing
  Exit Sub
...
Private Sub Grabar()
  ...
  Rs1.Fields.Append "PrecioIva", adInteger
  
  Rs1.Open
  Rs1.AddNew
  Rs1!Descripcion = TxtDescripcion.text
  Rs1!Porcentaje = TxtPorcentaje.text
  Rs1!PrecioIva = ChkPrecio.Value
  Rs1.Update
  
  Set cRse = New ClsEscritura
  
  If CmdBotones(2).Caption = "Grabar" Then
     cRse.Actualizar "ListaPrecioModif", Rs1, Val(Label3.Caption)
  Else
     cRse.Actualizar "ListaPrecioAgregar", Rs1
  End If
  Rs1.Close
  Set cRse = Nothing
  Exit Sub
