...
Begin VB.Form TblDescuentos 
   ...
   End
End
Attribute VB_Name = "TblDescuentos"
...
Private Sub LinkearTexto()
  Label3.Caption = Format(rs!Descuento, "00")
...
Private Sub List1_Click()
 ...
 Set rs = cRsl.TraerRS("DescuentoTraerUno", sCod, True)
 LinkearTexto
 Set cRsl = Nothing
Exit Sub
errHandler:
...
Private Sub Borrar()
  ...
  If cRsl.TraerCantidad("Clientes", "Descuento", "Descuento=" & Val(Label3.Caption)) = 0 _
      And cRsl.TraerCantidad("Registros", "Descuento", "Descuento=" & Val(Label3.Caption)) = 0 Then

    If MsgBox("Borra el Descuento Seleccionado ?", 20, "Atención") = 6 Then
       sCod = Label3.Caption
       cRse.Actualizar "DescuentoBorrar", Nothing, Val(Label3.Caption), "Descuento"
       CargarLista
       Limpiar
       Botones True, False, False, False, True
        If rs.EOF = False Or rs.BOF = False Then
    ...
    End If
  Else
     MsgBox "Descuento Asociado a un Cliente o es un Registro por defecto", vbCritical, "Atención"
  End If
  Exit Sub
errHandler:
  ManejaErrores
...
Private Sub CargarLista()
  ...
  Set cRsl = New ClsLectura
  Set rs = cRsl.TraerRS("DescuentoTraerTodos", Empty, True)

  Debug.Print "RS State:", rs.State, " EOF:", rs.EOF, " BOF:", rs.BOF
  ...
  If rs.State = 1 Then
      If (Not rs.BOF) And (Not rs.EOF) Then
          ...
          Do While Not rs.EOF
             ...
             List1.ItemData(List1.NewIndex) = rs!Descuento
             rs.MoveNext
          Loop
          rs.MoveFirst
      End If
  Else
      MsgBox "El recordset volvió CERRADO desde TraerRS()", vbCritical
  End If
...
Private Sub Grabar()
  ...
  If CmdBotones(2).Caption = "Grabar" Then
     cRse.Actualizar "DescuentoModif", Rs1, Val(Label3.Caption)
  Else
     cRse.Actualizar "DescuentoAgregar", Rs1
  End If
  Rs1.Close
  Set cRse = Nothing
  Exit Sub
