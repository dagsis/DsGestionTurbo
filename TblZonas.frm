...
Private Sub LinkearTexto()
  Label3.Caption = Format(Rs!Zona, "00")
...
Private Sub List1_Click()
 ...
 Set Rs = cRsl.TraerRS("ZonaTraerUno", sCod, True)
 LinkearTexto
 Set cRsl = Nothing
Exit Sub
errHandler:
...
Private Sub Borrar()
  ...
  If cRsl.TraerCantidad("Clientes", "Zona", "Zona=" & Val(Label3.Caption)) = 0 And cRsl.TraerCantidad("Registros", "Zona", "Zona=" & Val(Label3.Caption)) = 0 Then
    If MsgBox("Borra La Zona Seleccionada ?", 20, "Atención") = 6 Then
       sCod = Label3.Caption
       cRse.Actualizar "ZonaBorrar", Nothing, Val(Label3.Caption), "Zona"
       CargarLista
       Limpiar
       Botones True, False, False, False, True
        If Rs.EOF = False Or Rs.BOF = False Then
    ...
    End If
  Else
     MsgBox "Zona Asociada a un Cliente Especifico...", vbCritical, "Atención"
  End If
  Exit Sub
errHandler:
  ManejaErrores
...
Private Sub CargarLista()
  ...
  List1.Clear
  Set Rs = cRsl.TraerRS("ZonaTraerTodos", , True)

  Do While Not Rs.EOF
     List1.AddItem Rs!Descripcion
     List1.ItemData(List1.NewIndex) = Rs!Zona
     Rs.MoveNext
  Loop
  Set cRsl = Nothing
  Exit Sub
...
Private Sub Grabar()
  ...
  If CmdBotones(2).Caption = "Grabar" Then
     cRse.Actualizar "ZonaModif", Rs1, Val(Label3.Caption)
  Else
     cRse.Actualizar "ZonaAgregar", Rs1
  End If
  Rs1.Close
  bZona = True
  Set cRse = Nothing
