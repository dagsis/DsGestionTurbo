...
Private Sub LinkearTexto()
  Label3.Caption = Format(Rs!Familia, "00")
  TxtDescripcion.text = Rs!Descripcion
  TxtPorcentaje = Format(Rs!Porcentaje, nCantDecimales)
End Sub
...
Private Sub List1_Click()
 ...
 Set Rs = cRsl.TraerRS("FamiliaTraerUno", sCod, True)
 LinkearTexto
 Set cRsl = Nothing
Exit Sub
errHandler:
...
Private Sub Borrar()
  ...
  If cRsl.TraerCantidad("Productos", "Familia", "Familia=" & Val(Label3.Caption)) = 0 And _
     cRsl.TraerCantidad("Registros", "Familia", "Familia=" & Val(Label3.Caption)) = 0 Then
    If MsgBox("Borra La Familia Seleccionada ?", 20, "Atención") = 6 Then
       sCod = Label3.Caption
          cRse.Actualizar "FamiliaBorrar", Nothing, Val(Label3.Caption), "Familia"
          CargarLista
          Limpiar
          Botones True, False, False, False, True
          If Rs.EOF = False Or Rs.BOF = False Then
    ...
    End If
  Else
      MsgBox "Familia Asociada a un Producto Existente o Es un Registro por Defecto", vbCritical, "Atención"
  End If
  Exit Sub
errHandler:
  ManejaErrores
...
Private Sub CargarLista()
  ...
  List1.Clear
  Set Rs = cRsl.TraerRS("FamiliaTraerTodos", , True)

  Do While Not Rs.EOF
     List1.AddItem Rs!Descripcion
     List1.ItemData(List1.NewIndex) = Rs!Familia
     Rs.MoveNext
  Loop
  Set cRsl = Nothing
  Exit Sub
...
Private Sub Grabar()
  ...
  If CmdBotones(2).Caption = "Grabar" Then
     cRse.Actualizar "FamiliaModif", Rs1, Val(Label3.Caption)
  Else
     cRse.Actualizar "FamiliaAgregar", Rs1
  End If
  Rs1.Close
  Set cRse = Nothing
  bFamilia = True
