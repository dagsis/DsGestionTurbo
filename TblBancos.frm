...
Private Sub LinkearTexto()
  Label3.Caption = Format(Rs!Banco, "00")
...
Private Sub List1_Click()
 ...
 Set Rs = cRsl.TraerRS("BancoTraerUno", sCod, True)
 LinkearTexto
 Set cRsl = Nothing
Exit Sub
errHandler:
...
Private Sub Borrar()
  ...
  Dim nList As Integer, Rx As Recordset, cRx As ClsLectura

  Set cRse = New ClsEscritura
  Set cRx = New ClsLectura
  
  nList = List1.ListIndex
  If cRx.TraerCantidad("Caja", "Banco", "Banco=" & Val(Label3.Caption)) = 0 And _
     cRx.TraerCantidad("Registros", "Banco", "Banco=" & Val(Label3.Caption)) = 0 Then

    If MsgBox("Borra el Banco Seleccionado ?", 20, "Atención") = 6 Then
       sCod = Label3.Caption
       cRse.Actualizar "BancoBorrar", Rx, Val(Label3.Caption), "Banco"
       CargarLista
       Limpiar
       Botones True, False, False, False, True
       If Rs.EOF = False Or Rs.BOF = False Then
    ...
    End If
  Else
    MsgBox "Banco Asociado a un Movimiento o Registro por Defecto", vbCritical, "Atención"
  End If
  Exit Sub
errHandler:
  ManejaErrores
...
Private Sub CargarLista()
  ...
  List1.Clear
  Set Rs = cRsl.TraerRS("BancoTraerTodos", , True)

  Do While Not Rs.EOF
     List1.AddItem Rs!Descripcion
     List1.ItemData(List1.NewIndex) = Rs!Banco
     Rs.MoveNext
  Loop
  Set cRsl = Nothing
  Exit Sub
...
Private Sub Grabar()
  ...
  If CmdBotones(2).Caption = "Grabar" Then
     cRse.Actualizar "BancoModif", Rs1, Val(Label3.Caption)
  Else
     cRse.Actualizar "BancoAgregar", Rs1
  End If
  Rs1.Close
  bBanco = True
  Set cRse = Nothing
