VERSION 5.00
Begin VB.Form TblMoneda 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tablas de Monedas"
   ClientHeight    =   3405
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   6435
...
Private Sub LinkearTexto()
  ...
  TxtPorcentaje = Format(Rs!COTIZACION, "#0.0000")
End Sub
...
Private Sub List1_Click()
 ...
 Set Rs = cRsl.TraerRS("Monedas_T", sCod, True)
 LinkearTexto
 Set cRsl = Nothing
Exit Sub
errHandler:
...
Private Sub Borrar()
  ...
  If cRx.TraerCantidad("Productos", "Producto", "Moneda=" & Label3.Caption) = 0 _
          And cRx.TraerCantidad("Registros", "Moneda", "Moneda=" & Label3.Caption) = 0 Then

    If MsgBox("Borra La Moneda Seleccionada ?", 20, "Atención") = 6 Then
       sCod = Label3.Caption
       cRse.Actualizar "Monedas_B", Nothing, Val(Label3.Caption), "Moneda"
       CargarLista
       Limpiar
       Botones True, False, False, False, True
        If Rs.EOF = False Or Rs.BOF = False Then
    ...
    End If
  Else
     MsgBox "Moneda Asociada a un Articulo o al Registro por defecto", vbCritical, "Atención"
  End If
  Exit Sub
errHandler:
  ManejaErrores
...
Private Sub CargarLista()
  ...
  List1.Clear
  Set Rs = cRsl.TraerRS("Monedas_TT", , True)
...
Private Sub Grabar()
  ...
  Rs1.Fields.Append "Cotizacion", adDouble
  Rs1.Open
  Rs1.AddNew
  Rs1!Descripcion = TxtDescripcion.text
  Rs1!COTIZACION = TxtPorcentaje.text
  Rs1.Update
  
  Set cRse = New ClsEscritura
  
  If CmdBotones(2).Caption = "Grabar" Then
     cRse.Actualizar "Monedas_M", Rs1, Val(Label3.Caption)
  Else
     cRse.Actualizar "Monedas_A", Rs1
  End If
  Rs1.Close
  Set cRse = Nothing
  bFamilia = True
...
Private Sub TxtPorcentaje_LostFocus()
  ...
  TxtPorcentaje.text = Format(TxtPorcentaje.text, "#0.0000")
End Sub
